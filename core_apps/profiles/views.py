...
class ProfileDetailApiView(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        queryset = Profile.objects.select_related("user") # The selected_related method fetch the related objects as the same time as the main objects rather than issuing a seperate database query for each related object
        return queryset
...
class FollowerListView(APIView):
    ...
    def get(self, request, format = None):
        try:
            ...
            return Response({"Error": "Profile Does Not Exists!"}, status = status.HTTP_404_NOT_FOUND)
...
class FollowingListView(APIView):
    def get(self, request, user_id, format = None):
        try:
            ...
            following_profiles = profile.following.all() # the related name of followers field in Profile model is following
