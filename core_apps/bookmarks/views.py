...
class BookmarkDestroyView(generics.DestroyAPIView):
    ...
    def get_object(
        self,
    ):  # gets specific bookmark instance, which the user is trying to delete
        user = self.request.user
        article_id = self.kwargs.get("article_id")

        try:  # Validating whether the article ID is a valid UUID version 4
            UUID(
                str(article_id), version=4
            )  # We are converting our article ID to a string before passing it to the UUID function
        except ValueError:
            raise ValidationError("Invalid article_id provided")
