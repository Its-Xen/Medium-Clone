...
class TagListField(serializers.Field):
    ...
    def to_representation(self, value):
        return [
            tag.name for tag in value.all()
        ]  # return a list of tag names by iterating over all of the tags associated with the articles

    def to_internal_value(
        self, data
    ):  # the data here is the tags field from the request data
        ...
        for tag_name in data:
            tag_name = (
                tag_name.strip()
            )  # the strip method removes any trailing white spaces from the tag name.
