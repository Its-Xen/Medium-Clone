...
class ArticleListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        serializer.save(author = self.request.user) # we set the author field to request user
...
class ClapArticleView(generics.CreateAPIView, generics.DestroyAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if Clap.objects.filter(user = user, article = article).exists(): # check if the clap is already exists
        ...
        clap.save()
        return Response({"detail": "Clap added to article"}, status = status.HTTP_201_CREATED)
    ...
    def delete(self, request, *args, **kwargs):
        ...
        clap.delete()
        return Response({"detail": "Clap removed form article."}, status = status.HTTP_204_NO_CONTENT)