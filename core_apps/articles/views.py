...
class ArticleListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        )  # we set the author field to request user
        logger.info(
...
class ClapArticleView(generics.CreateAPIView, generics.DestroyAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if Clap.objects.filter(
            user=user, article=article
        ).exists():  # check if the clap is already exists
            return Response(
    ...
    def delete(self, request, *args, **kwargs):
        ...
        clap.delete()
        return Response(
            {"detail": "Clap removed form article."}, status=status.HTTP_204_NO_CONTENT
        )
